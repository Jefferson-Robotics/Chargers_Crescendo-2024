...
public class dockArmEncoderAuto extends CommandBase {
  ...
  public dockArmEncoderAuto(armSystem control) {
  ...
  public void execute() {
    ...
    if (state == 0) {
      if (control.moveBottom(1, Constants.bottomDockVertical) && control.moveTop(1, 0)) {
        state = 1;
      }
    } else if (state == 1) {
      if (control.moveBottom(0.3, -225)) {
        state = 2;
      }
    } else if (state == 2) {
      if (control.moveBottom(0.8, 0)) {
        isDone = true;
      }
    }
  }
